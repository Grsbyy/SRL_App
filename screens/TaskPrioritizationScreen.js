...
const TaskPrioritization = ({ navigation }) => {
  ...
  const calculatePrioritizationScore = (task) => {
    ...
    let prioritizationScore = ((dueDate - currentDate) / (1000 * 60 * 60)) + (24 - taskLength) + (5 - taskDifficulty);
    return prioritizationScore;
  };
  

  const addTasks = () => {
    if (!givenDate || !dueDate || !title || !rating || !len) {
      Alert.alert('Incomplete Entry', 'Please input Given date, Due date, and Task Name to submit.');
      return;
    }
        ...
        <View style={styles.centeredView}>
          <View style={styles.modalView}>
            ...
            <DateTimePickerModal
              ...
              mode="date"
              onConfirm={handleConfirmGivenDate}
